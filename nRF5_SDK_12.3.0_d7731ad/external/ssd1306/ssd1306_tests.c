...
void ssd1306_TestBorder() {
    ...
    uint32_t start = ticks * ( (APP_TIMER_PRESCALER + 1 ) * 1000 ) / APP_TIMER_CLOCK_FREQ;
    uint32_t end = start;
    uint8_t x = 0;
    uint8_t y = 0;
    do {
        ...
        sleep(5);
        end = app_timer_cnt_get() * ( (APP_TIMER_PRESCALER + 1 ) * 1000 ) / APP_TIMER_CLOCK_FREQ;
...
void ssd1306_TestFPS() {
    ...
    uint32_t start = app_timer_cnt_get() * ( (APP_TIMER_PRESCALER + 1 ) * 1000 ) / APP_TIMER_CLOCK_FREQ;
    uint32_t end = start;
    int fps = 0;
